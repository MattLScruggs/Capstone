...
ask_query<-function(existing_vector_db, pathogen){
  source('lit_review_functions_experiment.R')
  ...
  ### add centers to database ###
    ...
    center_1_string <- paste("What is the natural reservoir for ", query_string,"? Is ", query_string,"a zoonotic disease that jumped from another species through zoonosis? what animal species is",
                           query_string, "found in? Is ",query_string,"endemic in the human population? What historical accounts of ", query_string," outbreaks, epidemics, and plagues are there? ",
                           "Is ",query_string,"found in natural bodies of water or soil? What is the history of human infections with ",query_string,"?")
  
    
    
  
   #add centers to the database
    center1_embed<-add_new_strings_2(new_string = center_1_string, new_string_id = "C1", existing_vector_db = existing_vector_db)

    
    #get results
    qc1_results<-query_returns_2(existing_vector_db=existing_vector_db, center_query_df=center1_embed)
    

    #transmission of the pathogen
    center_2_string <- paste("How does ",query_string,"spread?", 
                             "how infectious is ",query_string,"?", 
                             "what is the R0 value of ",query_string,"?", 
                             "how long does it take for the case counts of ",query_string, "to double?", 
                             "Does an initial infection with ", query_string,"grant lifelong immunity?",
                             "How soon can reinfection with ", query_string, "occur?",
                             "What is the latent period of ", query_string,"?", 
                             "What is the contagious period of ", query_string,"?",  
                             "Do patients infected with ",query_string, "display symptoms before being contagious?")
    
    
    #add centers to the database
    center2_embed<-add_new_strings_2(new_string = center_2_string, new_string_id = "C2", existing_vector_db = existing_vector_db)
    
    
    #get results
    qc2_results<-query_returns_2(existing_vector_db=existing_vector_db, center_query_df=center2_embed)

    
    #symptomology
    center_3_string <- paste("what are the primary symptoms and clinical presentation of ",query_string,"?", 
                             "What is the timeline of infection for ",query_string,"?", 
                             "what is the case fatality rate or morbidity mortality rate of ",query_string,"?",
                             "what is the differential diagnosis of ",query_string,"?")
    
    
    #add centers to the database
    center3_embed<-add_new_strings_2(new_string = center_3_string, new_string_id = "C3", existing_vector_db = existing_vector_db)
    
    
    #get results
    qc3_results<-query_returns_2(existing_vector_db=existing_vector_db, center_query_df=center3_embed)
    
    
    #prevention and treatment methods
    center_4_string <- paste("How do you control the spread of ",query_string,"?", 
                             "Is ",query_string,"treatable with antibiotics?", 
                             "Does ",query_string," showcase any antibiotic resistance?", 
                             "what is the prognosis for ",query_string,"infection?",
                             "What percent of patients infected with ",query_string,"require hospitalization?", 
                             "Could control methods for ",query_string,"include handwashing, water purification, wearing masks?",
                            "is a vaccine for ",query_string," approved or in development?")
    
    
    #add centers to the database
    center4_embed<-add_new_strings_2(new_string = center_4_string, new_string_id = "C4", existing_vector_db = existing_vector_db)
    
    
    #get results
    qc4_results<-query_returns_2(existing_vector_db=existing_vector_db, center_query_df=center4_embed)
    

    domain_list<-c("Reservoir","Transmission","Clinical","Control Methods")
    score_list<-c(qc1_results[[2]], qc2_results[[2]],qc3_results[[2]],qc4_results[[2]])
    text_list<-c(qc1_results[[1]],qc2_results[[1]],qc3_results[[1]],qc4_results[[1]])
    epi_df<-data.frame(domain_list, score_list, text_list)
    colnames(epi_df)<-c("Domain","Best_Score","Text_Out")
...
query_returns_2<-function(existing_vector_db, center_query_df){
  ...
  top_scores<-row.names(score_df)[1]
  ...
  return_list<-list(match_out, score_df[1,1])
